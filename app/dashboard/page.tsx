...
export default function DashboardPage() {
  ...
  const filteredEntries = useMemo(() => {
    return entries.filter((entry) => {
      if (!showLowQuality && entry.distilled.quality_score < 0.4) return false
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-8">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-medium text-gray-700">{Math.round(filters.minQualityScore * 100)}%</span>
            </div>
            <input
              type="range"
              min="0"
          ...
          {entries.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-center border-2 border-dashed border-gray-200 rounded-xl bg-white/50">
              ...
              <p className="text-gray-500 max-w-sm mt-2 mb-6">
                Start by distilling a tweet thread or blog post to build your personal knowledge base.
              </p>
